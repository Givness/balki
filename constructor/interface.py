...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Кнопка запуска решения задачи (расчёт реакций и т.д.)
    ...
    def save_beam(self):
        ...
        if filename:  # Если пользователь выбрал файл
            ...
            try:
                self.grid_widget.beam.save_to_file(filename)  # Сохраняем балку
    ...
    def load_beam(self):
        ...
        try:
            self.clear_field()  # Очистка текущей балки
            self.grid_widget.beam = Beam.load_from_file(filename)  # Загрузка новой балки
            self.grid_widget.update()  # Перерисовка поля
        except Exception as e:
    ...
    def clear_field(self):
        # Сброс идентификаторов и списка используемых ID для всех типов объектов
        for cls in [Force, Torque, Support, Node, BeamSegment, Beam]:
            cls._next_id = 1           # Сброс счётчика ID
            cls._used_ids.clear()      # Очистка использованных ID
