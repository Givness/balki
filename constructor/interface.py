...
# Класс для координатной плоскости
class CoordinatePlane(QFrame):
    def __init__(self):
        ...
        self.lines = []  
        self.point_numbers = {}  
        self.squares = []  # Добавляем список для хранения квадратов
    ...
    def change_scale(self, factor):
        self.scale_factor *= factor  # Изменяем масштаб
        self.scale_factor = max(0.5, min(2.0, self.scale_factor))  # Ограничиваем диапазон от 0.5 до 2.0
    ...
    def paintEvent(self, event):
        try:
            ...
            for i, line in enumerate(self.lines, start=1):
                ...
                painter.drawEllipse(x1_pixel - 4, y1_pixel - 4, 8, 8)  # Точка начала
                painter.drawEllipse(x2_pixel - 4, y2_pixel - 4, 8, 8)  # Точка конца
                ...
                for (x, y), num in self.point_numbers.items():
                    ...
                    painter.drawText(x_pixel + 5, y_pixel - 5, str(num))  # Отображаем номер точки

            # Рисуем квадраты с углом
            for x, y, color, angle in self.squares:
                ...
                painter.save()  # Сохраняем состояние художника
                painter.translate(x_pixel, y_pixel)  # Перемещаем точку начала
                painter.rotate(-angle + 90)  # Поворачиваем
                painter.drawPixmap(-size // 2, -size // 2, size, size, QPixmap("support.svg"))
                painter.restore()

                # # Рисуем квадрат с учетом угла
                # painter.save()  # Сохраняем состояние художника
                # painter.translate(x_pixel, y_pixel)
                # painter.rotate(-angle)  # Поворачиваем
                # painter.drawRect(-size // 2, -size // 2, size, size)  # Рисуем квадрат
    ...
    def draw_line(self, x1, y1, x2, y2):
        try:
            color = random_color()  # Генерация случайного цвета для линии
            self.next_segment_number += 1
            self.lines.append((x1, y1, x2, y2, color))  # Добавляем линию с цветом
    ...
    def add_square(self, point_number, angle):
        for (x, y), num in self.point_numbers.items():
            if num == point_number:
                self.squares.append((x, y, random_color(), angle))  # Добавляем квадрат с цветом и углом
                self.update()
                break

    def save_as_image(self, file_path):
        pixmap = self.grab()  # Создаём снимок виджета
        pixmap.save(file_path, "JPG")  # Сохраняем в JPEG

    def draw_vector(self, x1, y1, x2, y2):
        color = random_color()  # Цвет вектора
        self.lines.append((x1, y1, x2, y2, color))  # Добавляем вектор как линию

        # Добавляем стрелку в конец вектора
        arrow_size = 0.2  # Размер стрелки
        dx, dy = x2 - x1, y2 - y1
        ...
        # Точки для стрелки (по бокам от конца линии)
...
class SquareInputDialog(QDialog):
    ...
    def get_data(self):
        ...
        angle = float(self.angle_input.text())  # Угол в градусах
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 800, 500)  # Размер окна
        ...
        menu_layout.setAlignment(Qt.AlignmentFlag.AlignTop)  # Кнопки сверху
        label = QLabel("Меню")
        ...
        menu_layout.addStretch()  # Растягиваем, чтобы кнопки были сверху

        # Область координатной плоскости
        self.plane = CoordinatePlane()

        # Добавляем в основной макет
        main_layout.addLayout(menu_layout, 1)  # Левая панель (1/4 ширины)
        main_layout.addWidget(self.plane, 3)  # Координатная плоскость (3/4 ширины)
    ...
    def on_button1_click(self):
        # Открываем диалоговое окно для ввода координат
        dialog = InputDialog()
        if dialog.exec() == QDialog.DialogCode.Accepted:
            x1, y1, x2, y2 = dialog.get_coordinates()
            self.plane.draw_line(x1, y1, x2, y2)  # Рисуем линию
