...
# Диалог отображения результатов решения
...
class DialogManager:
    ...
    def open_support_dialog(self):
        def apply(data):
            ...
            ufx = support_type_index != Support.Type.ROLLER.value  # Если не ролик — фиксировать X
            ut = support_type_index == Support.Type.FIXED.value    # Если жёсткая — фиксировать поворот
            self.grid_widget.node_mapping[node_number].add_support(
                Support(angle, 0, 0, 0, ufx, True, ut)
            )
    ...
    # Открытие окна с результатами решения
